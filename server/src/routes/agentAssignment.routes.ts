...
// Get datasets generated by a student's agent
router.get(
  '/:assignmentId/submissions/:submissionId/datasets',
  authenticateToken,
  requireInstructor,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const submissionId = parseInt(req.params.submissionId);
    ...
    const datasets = await agentAssignmentService.getDatasetsByAgentConfigId(
      submission.agentConfig!.id,
      req.user!.id,
      req.user!.isAdmin
    );
