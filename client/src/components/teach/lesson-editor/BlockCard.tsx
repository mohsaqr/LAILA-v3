...
/** Any icon component that takes a className (lucide icons + file-type icons). */
type IconComponent = ComponentType<{ className?: string }>;

export type BlockAccent = 'teal' | 'violet' | 'cyan' | 'sky' | 'amber' | 'rose';
 ...
 */
const ACCENTS: Record<BlockAccent, { light: AccentTokens; dark: AccentTokens }> = {
  teal: {
  ...
  cyan: {
    light: { bg: '#ecfeff', border: '#a5f3fc', chipBg: '#cffafe', fg: '#0e7490' },
...
interface BlockCardProps {
  ...
  accent: BlockAccent;
  title: string;
  /** Small pill to the right of the title, e.g. "Q1 of 3", "PDF", "Video". */
  badge?: string;
  /** Right-aligned action controls (change / delete buttons, etc.). */
