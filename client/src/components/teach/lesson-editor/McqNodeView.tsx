...
import { normalizeQuestion, blankQuestion, type McqQuestion } from './mcqShared';
 ...
 */
export const McqNodeView = ({ node, updateAttributes, deleteNode, editor }: NodeViewProps) => {
  const { t } = useTranslation(['teaching', 'common']);
  const { isDark } = useTheme();
  const editable = editor?.isEditable ?? true;

  const raw = (node.attrs.questions as McqQuestion[]) ?? [];
  const questions: McqQuestion[] = raw.length ? raw.map(normalizeQuestion) : [blankQuestion()];
  const n = questions.length;

  const cardBg = isDark ? '#1f2937' : '#ffffff';
  const cardBorder = isDark ? '#374151' : '#e5e7eb';
  const subtle = isDark ? '#e5e7eb' : '#374151';
  const muted = isDark ? '#9ca3af' : '#6b7280';
  const accent = isDark ? '#22d3ee' : '#0891b2';

  // ─── Author view ───────────────────────────────────────────────────────────
  if (editable) {
    const setQuestions = (next: McqQuestion[]) => updateAttributes({ questions: next });
    const patchQ = (qi: number, patch: Partial<McqQuestion>) =>
    ...
    const addOption = (qi: number) => patchQ(qi, { options: [...questions[qi].options, ''] });
...
const McqStudent = ({ questions, isDark }: { questions: McqQuestion[]; isDark: boolean }) => {
  const { t } = useTranslation(['teaching', 'common']);
  const n = questions.length;
  ...
  const check = () => setRevealed(prev => prev.map((r, i) => (i === cur ? true : r)));
  const restart = () => { setAnswers(questions.map(() => null)); setRevealed(questions.map(() => false)); setCur(0); setDone(false); };

  const optStyle = (oi: number): React.CSSProperties => {
    const base: React.CSSProperties = { backgroundColor: cardBg, borderColor: cardBorder, color: subtle };
    if (!isRevealed) {
      return oi === picked
        ? { ...base, borderColor: accent, backgroundColor: isDark ? 'rgba(34,211,238,0.10)' : '#ecfeff' }
        : base;
    }
