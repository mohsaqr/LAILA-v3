...
/** One file carried inside a folder. */
...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    lectureFolder: {
      insertLectureFolder: (attrs: { label: string; files: FolderFile[] }) => ReturnType;
    };
  }
}

/** Parse the data-files JSON safely into a FolderFile[]. */
const parseFiles = (raw: string | null): FolderFile[] => {
  if (!raw) return [];
  try {
