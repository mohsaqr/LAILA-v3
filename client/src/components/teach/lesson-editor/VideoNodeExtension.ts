...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    lectureVideo: {
      insertLectureVideo: (attrs: { src: string; mode?: 'file' | 'embed' }) => ReturnType;
    };
  }
}
    ...
    return [
      {
        ...
        getAttrs: el => ({
          ...
          mode: (el as HTMLElement).getAttribute('data-mode') ?? 'file',
        }),
      },
      {
        tag: 'video',
        getAttrs: el => ({ src: (el as HTMLElement).getAttribute('src') ?? '', mode: 'file' }),
      },
      {
        tag: 'iframe',
        getAttrs: el => ({ src: (el as HTMLElement).getAttribute('src') ?? '', mode: 'embed' }),
      },
    ];
  },
