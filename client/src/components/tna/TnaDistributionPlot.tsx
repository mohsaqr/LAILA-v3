      ...
      <svg width="100%" height={svgHeight} viewBox={`0 0 ${svgWidth} ${svgHeight}`} preserveAspectRatio="xMinYMin meet">
        <g transform={`translate(${margin.left},${margin.top})`}>
            ...
            return (
              <g key={t}>
                {labels.map(label => {
                  ...
                  return (
                    <rect key={label} x={t * (plotW / maxTimestep)} y={y}
                      width={barWidth} height={barH} fill={colorMap[label]} opacity={0.85}>
          ...
          {timesteps.map((_, t) => {
            ...
            return (
              <text key={t} x={t * (plotW / maxTimestep) + barWidth / 2} y={plotH + 14}
