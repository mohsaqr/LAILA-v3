...
# ---------------------------------------------------------------------------
# 5. Production Prisma schema (PostgreSQL)
# ---------------------------------------------------------------------------
SCHEMA_FILE="$SERVER_DIR/prisma/prod/schema.prisma"
info "Using production schema: $SCHEMA_FILE"
...
info "Generating Prisma client & applying schema..."
npx prisma generate --schema "$SCHEMA_FILE"

# Detect database state and apply schema safely
PSQL_DB_ARGS_SCHEMA=("${PSQL_ARGS[@]}" -d "$DB_NAME")
TABLE_COUNT=$(psql "${PSQL_DB_ARGS_SCHEMA[@]}" -tAc \
    "SELECT COUNT(*) FROM information_schema.tables
     WHERE table_schema = 'public'
       AND table_type = 'BASE TABLE'
       AND table_name != '_prisma_migrations'" 2>/dev/null || echo "0")

if [ -d "$SERVER_DIR/prisma/prod/migrations" ] && \
   [ -f "$SERVER_DIR/prisma/prod/migrations/migration_lock.toml" ]; then
    # Migrations directory exists with PostgreSQL lock — use migrate deploy
    info "Existing PostgreSQL migrations found — running migrate deploy..."
    npx prisma migrate deploy --schema "$SCHEMA_FILE"
    ok "Migrations applied"
elif [ "$TABLE_COUNT" -gt 0 ] 2>/dev/null; then
    # Database has user tables but no migrations directory — refuse destructive push
    die "Database '$DB_NAME' already has $TABLE_COUNT table(s) but no migrations directory.
    Running 'db push --accept-data-loss' on an existing database could destroy data.
    To fix this, create a baseline migration manually:
      cd $SERVER_DIR
      npx prisma migrate diff --from-empty --to-schema-datamodel prisma/prod/schema.prisma --script > baseline.sql
      npx prisma migrate resolve --applied <migration_name>
    Or, if you are sure the database can be wiped, drop all tables first and re-run this script."
else
    # Empty database — safe to push schema, then create baseline migration
    info "Empty database detected — applying schema with db push..."
    npx prisma db push --schema "$SCHEMA_FILE"
    ok "Schema applied to empty database"

    # Create a baseline migration so future deploys use 'migrate deploy'
    info "Creating baseline migration for future deploys..."
    MIGRATION_NAME="0_init"
    npx prisma migrate diff \
        --from-empty \
        --to-schema-datamodel prisma/prod/schema.prisma \
        --script > /tmp/laila_baseline.sql

    # Create migration directory and write the SQL
    MIGRATION_DIR="$SERVER_DIR/prisma/prod/migrations/$(date +%Y%m%d%H%M%S)_init"
    mkdir -p "$MIGRATION_DIR"
    mv /tmp/laila_baseline.sql "$MIGRATION_DIR/migration.sql"

    # Write the migration lock file for PostgreSQL
    cat > "$SERVER_DIR/prisma/prod/migrations/migration_lock.toml" <<'LOCKEOF'
# Please do not edit this file manually
# It should be added in your version-control system (i.e. Git)
provider = "postgresql"
LOCKEOF

    # Mark the baseline as already applied (schema is already in the DB)
    npx prisma migrate resolve --applied "$(basename "$MIGRATION_DIR")" --schema "$SCHEMA_FILE"
    ok "Baseline migration created and marked as applied"
fi
