...
# Remove SQLite migrations (incompatible with PostgreSQL) — one-time conversion
...
if [ -d "$SERVER_DIR/prisma/migrations" ] && \
   [ -f "$SERVER_DIR/prisma/migrations/migration_lock.toml" ]; then
    ...
    MIGRATION_DIR="$SERVER_DIR/prisma/migrations/$(date +%Y%m%d%H%M%S)_init"
    mkdir -p "$MIGRATION_DIR"
    mv /tmp/laila_baseline.sql "$MIGRATION_DIR/migration.sql"

    # Write the migration lock file for PostgreSQL
    cat > "$SERVER_DIR/prisma/migrations/migration_lock.toml" <<'LOCKEOF'
